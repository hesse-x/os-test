...
| 编号 | syscall | 签名 | 说明 |
|------|---------|------|------|
| 57 | sys_fork | `fork()` | 创建子进程（拷贝地址空间） |
| 58 | sys_execve | `execve(pathname, argv, envp)` | 替换进程映像 |
...
1. 实现 `sys_execve`（SYS_EXECVE = 58）：内核 open+read ELF → FD_CLOEXEC 关闭 → mm_release_pages → elf_load → 新栈 → 修改 trapframe
