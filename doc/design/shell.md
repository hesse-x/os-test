...
```nasm
; 极简 shell：打字回显 + 回车换行
; syscall 约定：eax=syscall号, ebx=arg1, int 0x80
...
当前 `process_create` 把 `init_code[]` 硬编码拷入用户代码页。需要增加一个加载路径：**从 ELF 加载**。

两种方式：

**A) 新增 `process_create_elf(const uint8_t *data, uint32_t size)`**

独立的创建函数，内部调用 `elf_load` 映射段，跳过 init_code 拷贝。`process_create(entry)` 保留给硬编码程序（调试用）。

**B) 改造 `process_create` 接受可选 ELF 数据**
