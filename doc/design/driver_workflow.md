...
3. 库初始化：`memfd_create("input_ring")` + `ftruncate(shm_fd, 4096)` 创建 input SHM，初始化 input_shm_header_t，`device_register_shm("kbd", shm_fd)` 绑定到 /dev/kbd inode
4. 主循环（库内）：
   - `sys_recv(&msg, NULL, 0, 0)` — 深度睡眠，等 EINTR（xHCI ISR wake kbd_openers[]）或 RECV_REQ
   - EINTR：`on_key_event()` 循环填 input_event_t → `broadcast_event()` 写 input SHM ring（head/tail 原子操作）+ 无条件 notify 所有 bound consumers
   - RECV_REQ：处理 INPUT_BIND（注册 consumer pid 到 `consumers[]`）/ INPUT_UNBIND

kbd_driver 自身不需要轮询窗口（数据源是硬件中断通知）。notify 无条件广播，无 sleeping flag（pid 失效时 notify 是 no-op）。
...
- **IPC/SHM**：动态 SHM 创建/attach 机制。详见 [ipc.md](kernel/ipc.md)
- **xHCI**：ISR 通过 USB HID SHM + kbd_openers[] wake 通知 kbd_driver。详见 [xhci.md](kernel/xhci.md)
- **KMS**：已迁入内核，terminal 通过 ioctl(FLIP) 请求 flip。详见 [kms.md](kernel/kms.md)
- **用户态驱动**：驱动进程架构总览。详见 [user_driver.md](user_driver.md)
- **键盘**：kbd_driver 3 层协议 + evdev-style input SHM。详见 [kbd.md](kbd.md)
