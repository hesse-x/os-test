...
sys_mmap（syscall #9）：纯匿名私有映射，内核自动选择虚拟地址（0x800000 起，mmap_brk 递增），size 按页对齐。扩展支持 MAP_PHYSICAL（详见 [boot.md](boot.md)）。
...
mmap_region 字段：vaddr / size / next。
...
- Page 描述符：kernel/mem/alloc.h : Page / PageStatus
