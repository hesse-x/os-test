...
| 分配器 | 位置 | 说明 |
|--------|------|------|
| Bump | arch/x64/paging.cc | 极简线性分配，kernel_end 起始，kernel_init_finish 后禁用 |
| BFC | kernel/mem/alloc.cc : BFCAllocator | Best-Fit Contiguous 页面级分配，frames[] + free_list + bfc_lock |
| 内核 slab | kernel/mem/slab.cc | per-CPU active slab + per-cache partial list |
...
sys_mmap（syscall #9）：纯匿名私有映射，内核自动选择虚拟地址（0x800000 起，mmap_brk 递增），size 按页对齐。扩展支持 MAP_PHYSICAL（详见 [boot.md](boot.md)）。mmap_region 新增 prot 字段（PROT_READ | PROT_WRITE | PROT_EXEC），sys_mmap 和 elf_load 时设置，fork COW 用 prot 判断页分类（详见 [page.md](page.md) COW 章节）。

sys_munmap（syscall #10）：遍历 mmap_regions 找匹配区域 → 逐页 unmap + 释放物理页 → 从链表删除 region → kfree(region)。
...
- BFC 分配器：kernel/mem/alloc.cc : BFCAllocator
- Page 描述符：kernel/mem/alloc.h : Page / PageStatus / p_refcount
- Slab 分配器：kernel/mem/slab.cc / slab.h
- 用户页映射：kernel/mem/user_mapping.cc : map_user_page_direct / unmap_user_pages
- copy_to_user / copy_from_user：kernel/mem/copy_user.cc
- 用户态 malloc：user/lib/malloc.cc
- sys_mmap / sys_munmap：kernel/proc.cc（或在 kernel/trap.cc）
...
| 共享 mmap（MAP_SHARED） | sys_mmap 加 flags 参数，多进程映射同一物理页，Wayland IPC 需要 | 高 |
