...
- [ ] libc: sprintf/snprintf/strtol/ctype/assert/getcwd/uname
...
| | `faccessat`（musl 自带版） | AT_EACCESS 路径要 `__block_all_sigs`/`__restore_sigs`/`__clone` clone shim | repo `faccessat`（`user/lib/file.cc`）已透传 flags（含 `AT_EACCESS`）直送 `sys_faccessat`，且内核 `do_faccessat` 已按 `AT_EACCESS` 选 effective uid 判，故 `eaccess`/`euidaccess` 已可用（见上行已落地）。musl 自带 `src/unistd/faccessat.c` 仍排除（`MUSL_UNISTD_EXCLUDE`）：其 `getuid()!=geteuid()` 时 fork+`__clone` checker 子进程用 real uid 判的路径需 clone shim，非 root 场景才触发；本 OS 进程默认 ruid==euid==0 走快路径，暂不补 |
| | `isatty` | musl 探 `TIOCGWINSZ`；串口（`driver/serial.c:211`）只答 `TCGETS`，余返 `-ENOTTY` | 串口补 `TIOCGWINSZ`（或 repo `isatty` 改用 `TCGETS`，已保留） |                                                                           
| | `sleep`/`usleep` | ~~musl 遇 `EINTR` 返剩余秒，repo 语义是 EINTR 后重试睡满~~ **[已落地 2026-08-02]** 切 musl `src/unistd/{sleep,usleep}.c`（POSIX 语义：EINTR 返回剩余秒）。全树无消费者依赖返回值（皆裸 `sleep(n);`/`usleep(n);`）；init/terminal crash 重启退避由 `waitpid` 重试循环兜底，提前返回无害。`MUSL_UNISTD_EXCLUDE` 拿掉 `sleep.c`/`usleep.c`，删 `time.cc` 整文件 |                                                                                                     
...
| | ~~`euidaccess`/`eaccess`~~ | ~~依赖 `faccessat(AT_EACCESS)`~~ | ✅ 已落地：新建 `modules/legacy.cmake`（单文件 `src/legacy/euidaccess.c`，`weak_alias(euidaccess,eaccess)`）编入 libc.a/libc.so，`libc.map` 导出。`euidaccess.c` 调 `faccessat(AT_EACCESS)`，链接到 repo `faccessat`（musl 自带 `faccessat.c` 仍排除于 `MUSL_UNISTD_EXCLUDE`——其 AT_EACCESS clone 路径需 `__block_all_sigs`/`__clone` shim，见下行）；内核 `do_faccessat` 按 `AT_EACCESS` 选 effective uid 判（见下「access/eaccess ruid/euid 分流」）。`secure_getenv` 同批落地（`stdlib.cmake` env 块加 `src/env/secure_getenv.c`，`libc.map` 导出；`libc.secure ? NULL : getenv`，本内核 auxv `AT_SECURE=0` 故常等价 getenv）。 |
...
退役 shim：`user/lib/time.cc` 已删除（`sleep`/`usleep` 随 unistd 批次切 musl，见上表 sleep/usleep 行）；删 `timespec_to_tm`（仓库非标准，0 调用者，musl 无对应，`libc.map`/`time.h` 同步删）。`user/include/time.h` 改转发 musl `include/time.h`（同 `sys/time.h` shim），拿到带 `__tm_gmtoff`/`__tm_zone` 的 `struct tm`（musl `gmtime_r`/`mktime`/`strftime` 写这些字段）。
