...
- **宽字符 w* 系列已排除**（非"顺带纳入"）：musl_stdio_objs glob **剔除 23 个 w* 文件**（`vfwprintf`/`vfwscanf`/`fgetwc`/`fputwc`/`fputws`/`fgetws`/`ungetwc`/`fwide`/`fwprintf`/`fwscanf`/`wprintf`/`wscanf`/`swprintf`/`swscanf`/`v{w,sw}{printf,scanf}`/`getwc`/`getwchar`/`putwc`/`putwchar`/`open_wmemstream`）。原因：这些文件引用 `iswspace`/`iswdigit`（宽 ctype，`src/ctype/isw*.c`）、`wcsnlen`（`src/string`）、`btowc`/`wctob`（`src/multibyte`）、C-locale 对象 `__c_locale`/`__c_dot_utf8_locale`（`src/locale/c_locale.c`）——本 OS 都没编进 libc。带上它们 libc.so 能链接（符号只是 UND），但动态加载器运行期重定位失败：`Error relocating /lib/ld-musl-x86_64.so.1: iswspace: symbol not found`（+ `wcsnlen`/`iswdigit`/`__c_locale`/`__c_dot_utf8_locale`）。本 OS 无宽字符 stdio 消费方且不发布 `<wchar.h>`（w* 声明所在），排除无损、且把 ctype/locale 依赖链挡在 libc 外。multibyte 同步缩到窄路径最小集（`wctomb`/`wcrtomb`/`mbrtowc`/`mbsinit`/`internal` 五文件，供 vfprintf `%ls`/vfscanf `%ls` link-time 引用；`MB_CUR_MAX`→`CURRENT_UTF8` 运行期读 `__pthread_self()->locale`，由 `__init_tp` 设，无 `__c_locale` link 依赖）。**解锁路径**：要用宽字符 stdio 时——发布 `<wchar.h>`、libc.map 加 `<wchar.h>` 块、补编 `src/ctype/isw*.c`+`src/string/wcsnlen.c`+`src/locale/c_locale.c`+`btowc`/`wctob`，再把 23 个 w* 文件从 exclude 名单移出。
