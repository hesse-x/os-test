...
struct cpu_local_t {
    ...
    uint64_t tss_rsp0;   // 本 CPU TSS 的 RSP0（schedule() 更新，SYSCALL 入口通过 %gs:32 读取）
    int run_count;        // 本 CPU 上可运行进程数（pick_cpu() 负载均衡用，当前未实现）
};
```
...
5. `sti`，进入 idle_entry（AP 参与调度，不再仅 hlt）
...
BKL 已在步骤 4 中完全移除，替换为细粒度锁。详见 [fine_grained_lock.md](fine_grained_lock.md)。

### 第二阶段：细粒度锁 ✅（当前方案）

- `scheduler_lock`：per-CPU，保护 `schedule()` 和运行队列（irqsave）
- `procs_lock`：全局，保护 `procs[]` 进程表
- `fb_lock`：全局，保护 framebuffer cursor + 缓冲区
- `irq_owner[]`：无锁，atomic read/write
...
- **当前状态**：AP 参与 schedule()，有独立 idle 进程和 run_queue，BKL 已移除
- `run.sh`：`-smp 2`

### 阶段四：SMP 调度 + BKL ✅（BKL 已移除，替换为细粒度锁）
- 实现自旋锁 ✅
- BKL：中断入口/出口加锁 ✅（已移除）
- 全局调度器 + 进程表加锁 ✅
- AP 进入调度循环 ✅
...
### 阶段五（优化）：BKL → 细粒度锁 ✅
- 拆分 BKL 为独立子系统锁 ✅
- ~~添加 IPI（reschedule_ipi, tlb_shootdown_ipi）~~（待实现）
- 验证并发正确性 ✅
