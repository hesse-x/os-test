...
- copy_to_user / copy_from_user / strncpy_from_user：kernel/xcore/mem/copy_user.c（`access_ok` 前置 + `_ASM_EXTABLE` 标注，唯一规范实现，正常/KASAN 共用）
- 异常表 fixup：kernel/xcore/mem/extable.h（`_ASM_EXTABLE` + `exception_table_entry`）+ kernel/xcore/trap.c : `fixup_exception` + build_script/linker.ld : `__ex_table` 段。机制详见 [kernel_exception.md](kernel_exception.md)「用户态指针保护」
