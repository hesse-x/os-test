...
- **per-fd refcount 保护对象生命周期**：RCU 读路径使用 `fd_lookup` + `file_get`（bump `f_count`）保护对象生命周期，操作完毕 `file_put` 释放。fd_table 已改为指针数组，`struct file` 有顶层 `f_count`，所有类型统一处理。CLONE_FILES 后共享 fd_table 时需确保 `file_get/file_put` 配对不遗漏。
- **offset 写回原子性**：当前 offset 写回**不持锁**（无 CLONE_FILES，单进程独占 fd_table），CLONE_FILES 后多线程共享同一 `struct file` 的 offset 需加 `spinlock_t f_pos_lock`（per-file，与 Linux 对齐），否则并发 write/lseek 产生数据竞争。
- **close + `synchronize_rcu`**：close 通过 `fd_uninstall` 移除指针后需 `synchronize_rcu()` 确保所有 RCU 读者退出，再 `file_put` 释放。当前单线程下天然安全，CLONE_FILES 后必须显式等待。
- **alloc_fd 持 fd_lock 期间不可调度**：当前内核不可抢占，`alloc_fd` 在 `fd_lock` 下扫描 fd_table 安全。抢占内核后需改为无锁扫描（RCU 读）+ `fd_lock` 仅覆盖 `fd_install` 窗口。
