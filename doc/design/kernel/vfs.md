...
| chmod/chown/mode 落盘策略 | FAT32 目录项只有 8-bit attr（只读/隐藏/系统/卷标/子目录/存档），无 Unix rwx 权限位。uid/gid/mode 仅存内存 inode cache（sys_fstat 读 inode 内存值，不再硬编码 uid=gid=0）。chmod/fchmod/fchmodat/chown/fchown/fchownat 已实现（落盘仅内存，与 utimensat Q5 一致）：直接在 syscall 层持 `i_lock` 改 `ip->mode/uid/gid`（**不新增 i_op 钩子**——`.setattr(ip,size)` 是 truncate 专用，内存态无磁盘写需求；未来 ACL 再加 `setattr_unix`），写 ctime 经 `i_op->update_time`/generic 回退。setuid 位清除规则（安全基石）：非特权（无 CAP_FSETID）chmod 清 S_ISUID/S_ISGID、清目录 S_ISVTX；非特权 chown 清 S_ISREG 的 S_ISUID/S_ISGID；root（有 CAP_FSETID）保留——sudo 使能前提（位需能设上，execve 处理 S_ISUID 已实现——见 [proc.md](proc.md) execve 段：close 前捕获 mode/uid/gid + m_flags，预计算 new_euid/egid，auxv 用新值，point-of-no-return 后提交；`MS_NOSUID` 跳过）。权限收口：chmod 经 `capable(CAP_FOWNER)` 放行否则 euid 须匹配 owner（非 owner 返 EPERM）；chown 本轮简化为 `capable(CAP_CHOWN)`（root-only），Linux 复杂规则留 todo。FAT32 vs tmpfs 持久性差异同 utimensat：fat32 inode 无 fs 内部强引用，path chmod/chown 释放后可被回收，再 stat 建新 inode 回默认值；tmpfs 子项持 inode 引用不回收，path 设置+path 验证精确往返（fd 路径经 open 持引用，FAT32 亦可）。umask getter/setter 已实现，fat32_open 创建文件时未读取 umask（见待完成项 umask 接入） | 中 |
