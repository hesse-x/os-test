...
自上而下 POSIX 功能验证，采用 Unity 测试框架，按子系统分 ELF 组织，串口 log 输出结果。覆盖 58 个 syscall（编号 0-57）及 libc 用户态库。
...
| 构建集成 | Unity 作为独立 libunity.a，LINK_LIBS c unity | 不污染 libc.a，add_user_elf 已支持 LINK_LIBS |
...
```
1. 实现 setjmp/longjmp（x86-64，~50行汇编）       → 解锁 Unity 测试隔离
2. 集成 Unity 到 CMake 构建（libunity.a）          → 3 个文件加入 user/lib/unity/
3. 实现 test_runner.elf                            → spawn + waitpid 收集退出码
4. Layer 1 测试：test_pipe.c                       → 所有后续测试的基础
5. Layer 2-11 按依赖关系逐层推进
6. 迁移现有临时测试（malloctest/writetest/pcie/pthread_test）到 Unity 框架
```
...
| signal (kill/sigaction/sigreturn/tgkill) | `user/test/test_signal.c` | `signal.elf` | c unity | — |
| poll | `user/test/test_poll.c` | `poll.elf` | c unity | — |
| pthread (clone/futex/arch_prctl/set_tid_address/gettid) | `user/test/test_pthread.c` | `pthread.elf` | c unity | `user/pthread_test.c`（create/mutex/join） |
| IPC (shm_create/shm_attach/notify/req/resp/msg/msg_resp) | `user/test/test_ipc.c` | `ipc.elf` | c unity | — |
| process (spawn/waitpid/exit/execv) | `user/test/test_process.c` | `process.elf` | c unity | — |
...
for elf in build/pipe.elf build/string.elf build/malloc.elf build/stdio.elf \
           ...
           build/poll.elf build/pthread.elf build/ipc.elf build/process.elf \
           build/pci.elf build/test_runner.elf; do
...
Layer 7: process (spawn/waitpid/exit/execv)  ← 进程管理
Layer 8: signal (kill/sigaction/sigreturn/tgkill) ← 信号
Layer 9: poll                                ← Wayland 前置
Layer 10: pthread (clone/futex/arch_prctl/set_tid_address/gettid) ← 线程
Layer 11: PCI/device (pci_dev_info/open_dev/ioperm/block_io) ← 硬件驱动接口
```

## Syscall 覆盖范围

当前 58 个 syscall（编号 0-57），按测试子系统分组：

| 测试子系统 | 覆盖的 syscall | 未覆盖 syscall |
|-----------|---------------|---------------|
| unistd | getpid(0), yield(1), pipe(14), write(15), read(16), close(17) | — |
| fcntl | open(install_fd 33), dup2(25), fcntl(26), lseek(44) | — |
...
| process | spawn(8), waitpid(7), exit(6), exit_group(55), execv(51) | — |
| signal | kill(47), sigaction(48), sigreturn(49), tgkill(54) | — |
| poll | poll(43) | — |
| pthread | clone(50), futex(52), arch_prctl(53), set_tid_address(56), gettid(57) | — |
| PCI/device | pci_dev_info(29), open_dev(32), ioperm(24), block_io(30), block_async(31) | — |
| 驱动专用 | irq_bind(5), recv(2), load_dev(18), dma_alloc(27), dma_free(28) | 不纳入通用 POSIX 测试，驱动自有验证 |

**未覆盖的 syscall**（驱动专用，不纳入通用测试）：
- `recv(2)` — 内核消息接收，驱动进程内部使用
- `irq_bind(5)` — IRQ 绑定，驱动进程使用
- `load_dev(18)` — 设备加载，init 使用
- `dma_alloc(27)` / `dma_free(28)` — DMA 内存，AHCI 驱动使用
...
| # | Case 名 | 测什么 |
|---|---------|--------|
| 1 | mmap_anon | `mmap(NULL, 4096, PROT_READ|PROT_WRITE, MAP_ANONYMOUS|MAP_PRIVATE, -1, 0)` 返回非 NULL |
| 2 | mmap_write_read | mmap 页写入数据 → 读回验证 |
| 3 | mmap_multi_page | mmap 3 页 (12288 字节) → 各页独立读写 |
| 4 | munmap_basic | `munmap(addr, size)` 释放 → 再访问应 fault（间接验证） |
| 5 | mmap_addr_hint | `mmap(hint_addr, ...)` 使用地址提示 |
| 6 | mmap_shm_fd | `shm_create` → `mmap(fd, MAP_SHARED)` → 写入 → 另进程 attach 读回 | 是（shm attach） |
| 7 | memfd_create | `memfd_create("test", 0)` 返回 fd >= 0 |
| 8 | memfd_mmap | memfd → ftruncate → mmap → 写读验证 |
| 9 | ftruncate_grow | `ftruncate(fd, 8192)` 扩大 memfd → mmap 成功 |
| 10 | mmap_prot_exec | `mmap(PROT_READ|PROT_EXEC)` 代码页可执行（间接验证） |
...
| 7 | execv_replace | `execv(pathname, elf_data, elf_size)` 替换当前进程映像 | 是 |
...
| 5 | tgkill_specific | `tgkill(tgid, tid, SIGTERM)` 向特定线程发信号 | 是（需 pthread） |
| 6 | sigreturn | handler 执行后 sigreturn 恢复上下文 | 是 |
...
## Layer 10: test_pthread.c 详细 case 列表（迁移自 pthread_test.c）

| # | Case 名 | 测什么 |
|---|---------|--------|
| 1 | pthread_create | 创建线程，线程函数执行并返回 |
| 2 | pthread_join_return | `pthread_join(tid, &ret)` 获得线程返回值 |
| 3 | pthread_mutex_lock_unlock | mutex 互斥保护共享变量 |
| 4 | pthread_mutex contention | 两线程争锁 → 串行化访问共享 counter |
| 5 | pthread_create_multiple | 创建 3+ 线程，全部 join 完成 |
| 6 | gettid_basic | `gettid()` 主线程返回 tid > 0 |
| 7 | gettid_diff_thread | 不同线程 gettid() 返回不同 tid |
| 8 | sched_yield_thread | `sched_yield()` 让其他线程运行 |
| 9 | futex_wait_wake | `futex(addr, FUTEX_WAIT, val)` + `futex(addr, FUTEX_WAKE, 1)` 基本唤醒 |
| 10 | arch_prctl_set_fs | `arch_prctl(ARCH_SET_FS, addr)` 设置线程 local storage |
| 11 | clone_vm | `clone(CLONE_VM|CLONE_FILES|CLONE_THREAD)` 共享地址空间 |
| 12 | set_tid_address | `set_tid_address(&tid)` 设置 tid 清除地址 |

## Layer 11: test_pci.c 详细 case 列表（迁移自 test_pcie.c）
...
| `user/pthread_test.c` | pthread create/mutex/join | test_pthread.c | 待迁移 |

CMake 中已有 `add_user_elf` 目标（hello/malloc/pcie/write/pthreadtest），迁移后统一改为 `user/test/` 目录下的规范测试目标，LINK_LIBS 加入 `unity`。
...
- **多线程测试**：Unity 本身不支持多线程执行（全局状态非线程安全），但 `test_pthread.elf` 内部可用 pthread API 测试多线程功能
