...
共享内存已重构为 fd 模型，对齐 Linux memfd_create + mmap + SCM_RIGHTS 语义。
...
**struct shm**（kernel/proc.h : shm）
  ...
  ref_count : int — 引用计数：fd +1，mmap vma +1
  flags : int — SHM_KERNEL | SHM_SEALED
  seals : uint32_t — F_SEAL_SHRINK / F_SEAL_GROW / F_SEAL_WRITE / F_SEAL_SEAL
  name[32] : char — memfd_create 传入的调试名
  page_list : uint64_t* — NULL 表示 phys 连续，否则每个 entry 是 4K 页物理地址
...
**mmap_region**（kernel/proc.h : mmap_region）
  ...
  shm_obj : shm* — mmap(SHM fd) 时非 NULL，其 phys/npages 覆盖 phys/size
  next : mmap_region*

### 引用计数生命周期

创建时 ref_count=1（fd 持有），mmap +1，close(fd) -1，munmap -1，SCM_RIGHTS 发送 +1，dup2 +1。进程 exit 遍历 fd_table 和 mmap_regions 各减一次。ref_count=0 时 kfree(shm) + 释放物理页。
...
SHM_KERNEL 标志（flags 位）：页由内核管理，ref_count==0 时不释放物理页。内核初始化时创建 USB HID SHM（ref_count=0），kbd_driver 通过 sys_shm_attach 获取 fd + mmap 映射，close/munmap 后 ref_count 归零但物理页保留。

### syscall 接口

- sys_shm_create(size) → fd（syscall #11）— 类似 memfd_create
- sys_shm_attach(id, mode) → fd（syscall #12）— 过渡用，等 SCM_RIGHTS 就绪后删除
- sys_mmap(addr, size, prot, flags, fd, offset)（syscall #9）— MAP_SHARED+fd≥0 映射 SHM fd；MAP_ANONYMOUS 匿名映射；MAP_PHYSICAL 物理地址映射

### memfd_create + ftruncate + sealing

已实现对齐 Linux memfd_create 完整签名：

- sys_memfd_create(name, flags)（syscall #41）— 返回 fd，size=0；MFD_CLOEXEC 存入 fd flags；MFD_ALLOW_SEALING 允许后续加 seal
- sys_ftruncate(fd, size)（syscall #42）— 扩大：分配新物理页；缩小：释放超出的物理页（受 F_SEAL_SHRINK 限制）
- sys_fcntl(fd, cmd, arg)（syscall #24）— F_ADD_SEALS(1033)/F_GET_SEALS(1034)
...
seal 一经设置不可撤销。兼容：sys_shm_create(size) 保留作为快捷方式（= memfd_create + ftruncate 两步）。

### proc_reap 清理路径

遍历 fd_table：type==FD_SHM → shm_put（ref_count--，归零时 kfree+释放页）。遍历 mmap_regions：shm_obj != NULL → shm_put。SHM 页只 unmap PTE，不释放物理页。
...
| 30-39 | socket 系列 | socket/bind/listen/accept/connect/socketpair/sendmsg/recvmsg/shutdown/poll |
| 40 | sys_lseek | 文件偏移设置 |
...
| 47-56 | VFS 系列 | open/stat/mkdir/unlink/rmdir/dev_create/getdents/ioctl/fstat/fdev_pid |
| 57 | sys_fork | fork |
| 58 | sys_execve | execve |
| 59 | sys_setsid | 创建新会话 |
| 60 | sys_setpgid | 设置进程组 |
| 61 | sys_getpgid | 获取进程组 |
| 62 | sys_getsid | 获取会话 ID |
