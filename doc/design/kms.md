...
请求方写 N 条命令到 `cmds[]`，设 `count = N`，然后 `sys_notify(sys_lookup_dev(DEV_KMS))`。KMS 驱动读 `count`，处理所有命令，清零 `count`。
...
驱动 PID 通过 `sys_lookup_dev(dev_type)` 动态发现（`common/dev.h` 定义设备类型），所有驱动 include 该头文件。详见 [dev_table.md](dev_table.md)。
...
- `sys_putc` 已完全删除，编号已紧凑重排（当前 NR_SYSCALL=20，编号 0-19）
- `fb_lock` 同步删除
...
- 删除 `sys_putc` 输出路径
- 新增 `kms_flush` 函数：向 KMS_REQ 写命令 → `sys_notify(sys_lookup_dev(DEV_KMS))`
- libc 的 `kms_write_flush`：stdout/stderr 的 `write_fn` 改为写 KMS_REQ + 通知 KMS 驱动

## libc stdout 路径

- libc 的 `kms_write_flush` 替代原 `sys_putc_flush`：stdout/stderr 的 `write_fn` 改为向 KMS_REQ 共享页写入 PUTC 命令 + `sys_notify(sys_lookup_dev(DEV_KMS))`
