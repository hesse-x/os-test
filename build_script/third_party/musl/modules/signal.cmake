...
# syscalls cross a wire-ABI boundary. The kernel's sys_sigaction wire now
# matches musl's struct k_sigaction (arch/x86_64/ksigaction.h: handler/flags/
# restorer/mask[2], 40 bytes) and accepts any sigsetsize >= 8 (reading only the
# low 8 bytes of mask[]), so musl's upstream src/signal/sigaction.c is compiled
# here directly — its __libc_sigaction/__get_handler_set are the real sigaction
# backend (weak_alias __sigaction → sigaction). signal.cc keeps the thin
# wrappers kill/raise/signal/sigprocmask/sigpending/sigaltstack/sigreturn that
# are NOT defined here (no multi-define); signal() routes through the public
# sigaction() (musl). Therefore this module compiles musl signal sources that
# are pure-userspace or thin-syscall AND not already defined in signal.cc
# (no multi-define) AND not touching the wire sigaction struct, PLUS sigaction.c
# itself (the now-unglued backend).
#
# SAFE set compiled here (none defined in signal.cc, none multi-define):
#   sigaction     — upstream __libc_sigaction/__sigaction/weak_alias sigaction.
#                   Wire ABI now matches musl k_sigaction; no glue needed.
...
#                   musl __libc_sigaction, link+runtime OK) + sigprocmask.
...
#   raise/signal/kill/sigprocmask/sigpending/sigaltstack — already
...
add_musl_lib(musl_signal_objs SOURCES
    ${MUSL_DIR}/src/signal/sigaction.c
