...
static void vt100_csi_dispatch(char final_ch) {
  switch (final_ch) {
  case 'H':
  case 'f': { // Cursor position: \033[row;colH
...
int main(int argc, char **argv, char **envp) {
  ...
  // Access driver's SHM through /dev/kbd inode (kernel sys_mmap FD_DEV +
  // ip->shm path).
  volatile void *input_shm =
      mmap(NULL, 4096, PROT_READ | PROT_WRITE, MAP_SHARED, kbd_fd, 0);
  ...
  while (1) {
    ...
    for (int ei = 0; ei < nev; ei++) {
      uint8_t ascii_buf[4];
      int ascii_len =
          input_event_to_ascii(&evs[ei], ascii_buf, sizeof(ascii_buf));
      if (ascii_len <= 0)
        continue; // non-ASCII (modifier/extended) — ldisc handles via caller
