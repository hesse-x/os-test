...
// user/lib/start_main.cc — __libc_start_main unified startup (single source,
// dual products) ld.md §3.5.2 / plan_ld2b3 T11
//
// libc.a is compiled with -DDYNAMIC=0 (static path calls __libc_tls_init)
// libc.so is compiled with -DDYNAMIC=1 (dynamic path calls
...
// Static-path TLS initialization (tls.cc): read linker symbols to fill
// g_tls_info + alloc TCB + FS_BASE + ...
...
// (The original SysV ABI's init/fini/rtld_fini/stack_end are no longer used;
// the registers are repurposed to pass the ranges) De facto ABI: crt0.S calls
// across .so boundaries, so it must be exported (explicitly default under
// -fvisibility=hidden)
...
__libc_start_main(int (*main)(int, char **, char **), int argc, char **argv,
  ...
  __libc_tls_init_rest();
#else
  // Static: __libc_tls_init does it all (first fills g_tls_info + rest allocs
  // TCB + ...)
  __libc_tls_init();
#endif
  ...
  // rtld_fini (ld.so's fini): in this design ld.so does not register it, pass
  // NULL
