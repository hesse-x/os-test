...
static const uint16_t hid_to_input_key[256] = {
    ...
    0, 0,
    // 0x48-0x4F
    0, KEY_INSERT, KEY_HOME, KEY_PAGEUP, KEY_DELETE, KEY_END, KEY_PAGEDOWN, KEY_RIGHT,
    // 0x50-0x57
...
static uint8_t hid_modifier_to_mods(uint8_t hid_mods) {
    uint8_t mods = 0;
    if (hid_mods & 0x01) mods |= MOD_CTRL;   // Left Ctrl
    if (hid_mods & 0x02) mods |= MOD_SHIFT;   // Left Shift
    if (hid_mods & 0x04) mods |= MOD_ALT;     // Left Alt
    if (hid_mods & 0x10) mods |= MOD_CTRL;    // Right Ctrl
    if (hid_mods & 0x20) mods |= MOD_SHIFT;   // Right Shift
    if (hid_mods & 0x40) mods |= MOD_ALT;     // Right Alt
    if (hid_mods & 0x08) mods |= MOD_CAPS;    // Treat Left GUI as Caps (approximation)
    return mods;
}
...
int get_keycode(key_event *ev) {
    ...
    volatile struct usb_hid_slot *slot =
        (volatile struct usb_hid_slot *)(hid_shm_base + HID_SUBRING_KBD_OFFSET + tail * HID_SLOT_SIZE);

    // Only process keyboard type
    if (slot->type != HID_TYPE_KEYBOARD) {
        // Skip non-keyboard slot
        __atomic_store_n(&hid_hdr->rings[0].tail, (tail + 1) % HID_SUBRING_CAPACITY, __ATOMIC_RELEASE);
    ...
    for (int i = 0; i < 6; i++) current_keys[i] = slot->data[2 + i];

    uint8_t last_mods = last_report[0];
    uint8_t last_keys[6];
    for (int i = 0; i < 6; i++) last_keys[i] = last_report[2 + i];

    // Advance tail
    __atomic_store_n(&hid_hdr->rings[0].tail, (tail + 1) % HID_SUBRING_CAPACITY, __ATOMIC_RELEASE);

    // Update last_report
    for (int i = 0; i < 8; i++) last_report[i] = slot->data[i];

    // Detect events:
    // 1. Modifier changes — if modifier bitmap changed, report modifier press/release
    ...
    if (mod_diff) {
        ...
        for (int bit = 0; bit < 8; bit++) {
            if ((mod_diff >> bit) & 1) {
                ...
                switch (bit) {
                    case 0: ev->key = KEY_LEFTCTRL; break;
                    case 1: ev->key = KEY_LEFTSHIFT; break;
                    case 2: ev->key = KEY_LEFTALT; break;
                    case 5: ev->key = KEY_RIGHTSHIFT; break;
                    case 6: ev->key = KEY_RIGHTALT; break;
                    case 4: ev->key = KEY_RIGHTCTRL; break;
                }
                ev->pressed = pressed;
    ...
    for (int i = 0; i < 6; i++) {
        if (current_keys[i] == 0) continue;
        int found_in_last = 0;
        for (int j = 0; j < 6; j++) {
            if (current_keys[i] == last_keys[j]) { found_in_last = 1; break; }
        }
        if (!found_in_last) {
            ev->key = hid_to_input_key[current_keys[i]];
            ev->pressed = 1;
            ev->modifiers = hid_modifier_to_mods(current_mods);
            if (ev->key == 0) continue;  // unmapped key, try next
            return 0;
        }
    }
    ...
    for (int i = 0; i < 6; i++) {
        ...
        for (int j = 0; j < 6; j++) {
            if (last_keys[i] == current_keys[j]) { found_in_current = 1; break; }
        }
        if (!found_in_current) {
