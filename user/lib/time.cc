...
static void secs_to_tm(time_t t, struct tm *tm) {
    ...
    for (;;) {
        ...
        else if (days < 0) { year--; leap = ((year % 4 == 0 && year % 100 != 0) || (year % 400 == 0)); days += (leap ? 366 : 365); }
        else { ydays = days; break; }
    }
...
time_t mktime(struct tm *tm) {
    ...
    long long y = 1970;
    if (year >= 1970) {
        while (y < year) { days += is_leap(y) ? 366 : 365; y++; }
    } else {
        while (y > year) { y--; days -= is_leap(y) ? 366 : 365; }
    }
    /* 累加月天数到 mon */
    for (int m = 0; m < mon; m++) days += month_days(year, m);
...
static int put_num(char **p, char *end, long long val, int width, int pad) {
    ...
    int n = 0;
    if (val == 0) tmp[n++] = '0';
    while (val > 0 && n < 31) { tmp[n++] = '0' + (int)(val % 10); val /= 10; }
...
size_t strftime(char *buf, size_t max, const char *fmt, const struct tm *tm) {
    ...
    static const char *mon_name[12] = {"January","February","March","April","May",
    ...
    while (*fmt && p < end - 1) {
        ...
        fmt++;
        switch (*fmt) {
        case 'Y': put_num(&p, end, (long long)tm->tm_year + 1900, 4, '0'); break;
        case 'm': put_num(&p, end, tm->tm_mon + 1, 2, '0'); break;
        case 'd': put_num(&p, end, tm->tm_mday, 2, '0'); break;
        case 'H': put_num(&p, end, tm->tm_hour, 2, '0'); break;
        case 'M': put_num(&p, end, tm->tm_min, 2, '0'); break;
        case 'S': put_num(&p, end, tm->tm_sec, 2, '0'); break;
        case 'j': put_num(&p, end, tm->tm_yday + 1, 3, '0'); break;
        case 'w': put_num(&p, end, tm->tm_wday, 1, '0'); break;
        case 'Z': put_str(&p, end, "UTC"); break;
        case 'z': put_str(&p, end, "+0000"); break;
        case 'A': put_str(&p, end, wday_name[tm->tm_wday % 7]); break;
        case 'B': put_str(&p, end, mon_name[tm->tm_mon % 12]); break;
        case 'p': put_str(&p, end, tm->tm_hour < 12 ? "AM" : "PM"); break;
        case 'T': put_num(&p, end, tm->tm_hour, 2, '0'); *p++ = ':';
                  put_num(&p, end, tm->tm_min, 2, '0'); *p++ = ':';
                  put_num(&p, end, tm->tm_sec, 2, '0'); break;
        case 'F': put_num(&p, end, (long long)tm->tm_year + 1900, 4, '0'); *p++ = '-';
                  put_num(&p, end, tm->tm_mon + 1, 2, '0'); *p++ = '-';
                  put_num(&p, end, tm->tm_mday, 2, '0'); break;
        case 'R': put_num(&p, end, tm->tm_hour, 2, '0'); *p++ = ':';
                  put_num(&p, end, tm->tm_min, 2, '0'); break;
        case 'D': put_num(&p, end, tm->tm_mon + 1, 2, '0'); *p++ = '/';
                  put_num(&p, end, tm->tm_mday, 2, '0'); *p++ = '/';
                  put_num(&p, end, (tm->tm_year + 1900) % 100, 2, '0'); break;
        case 's': put_num(&p, end, (long long)mktime((struct tm *)tm), 0, ' '); break;
...
char *asctime_r(const struct tm *tm, char *buf) {
    ...
    tmp[n++] = wd[0]; tmp[n++] = wd[1]; tmp[n++] = wd[2]; tmp[n++] = ' ';
    tmp[n++] = mo[0]; tmp[n++] = mo[1]; tmp[n++] = mo[2]; tmp[n++] = ' ';
    /* dd 用空格填充（asctime 语义：%e 风格） */
    if (d < 10) { tmp[n++] = ' '; tmp[n++] = '0' + d; }
    else { tmp[n++] = '0' + d / 10; tmp[n++] = '0' + d % 10; }
    tmp[n++] = ' ';
    tmp[n++] = '0' + h / 10; tmp[n++] = '0' + h % 10; tmp[n++] = ':';
    tmp[n++] = '0' + mi / 10; tmp[n++] = '0' + mi % 10; tmp[n++] = ':';
    tmp[n++] = '0' + s / 10; tmp[n++] = '0' + s % 10; tmp[n++] = ' ';
    tmp[n++] = '0' + (y / 1000) % 10;
    tmp[n++] = '0' + (y / 100) % 10;
    tmp[n++] = '0' + (y / 10) % 10;
    tmp[n++] = '0' + y % 10;
