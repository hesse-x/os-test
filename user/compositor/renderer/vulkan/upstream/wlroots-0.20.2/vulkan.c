...
static VKAPI_ATTR VkBool32 debug_callback(
    ...
    const VkDebugUtilsMessengerCallbackDataEXT *debug_data, void *data) {
...
struct wlr_vk_instance *vulkan_instance_create(bool debug) {
  ...
  if (debug_utils_found) {
    ...
    if (ini->api.createDebugUtilsMessengerEXT) {
      ini->api.createDebugUtilsMessengerEXT(ini->instance, &debug_info, NULL,
                                            &ini->messenger);
    } else {
...
void vulkan_instance_destroy(struct wlr_vk_instance *ini) {
  ...
  if (ini->messenger && ini->api.destroyDebugUtilsMessengerEXT) {
    ini->api.destroyDebugUtilsMessengerEXT(ini->instance, ini->messenger, NULL);
  }
...
static void log_phdev(const VkPhysicalDeviceProperties *props) {
  ...
  wlr_log(WLR_INFO, "  Supported API version: %u.%u.%u", vv_major, vv_minor,
          vv_patch);
  wlr_log(WLR_INFO, "  Driver version: %u.%u.%u", dv_major, dv_minor, dv_patch);
}

VkPhysicalDevice vulkan_find_drm_phdev(struct wlr_vk_instance *ini,
                                       int drm_fd) {
  VkResult res;
  uint32_t num_phdevs;
  ...
  for (uint32_t i = 0; i < num_phdevs; ++i) {
    ...
    uint32_t avail_extc = 0;
    res = vkEnumerateDeviceExtensionProperties(phdev, NULL, &avail_extc, NULL);
    if ((res != VK_SUCCESS) || (avail_extc == 0)) {
    ...
    VkExtensionProperties avail_ext_props[avail_extc + 1];
    res = vkEnumerateDeviceExtensionProperties(phdev, NULL, &avail_extc,
                                               avail_ext_props);
    if (res != VK_SUCCESS) {
      wlr_vk_error("  Could not enumerate device extensions", res);
      continue;
    ...
    bool has_drm_props = check_extension(
        avail_ext_props, avail_extc, VK_EXT_PHYSICAL_DEVICE_DRM_EXTENSION_NAME);
    bool has_driver_props = check_extension(
        avail_ext_props, avail_extc, VK_KHR_DRIVER_PROPERTIES_EXTENSION_NAME);
    ...
    if (has_driver_props) {
      wlr_log(WLR_INFO, "  Driver name: %s (%s)", driver_props.driverName,
    ...
    if (drm_fd >= 0) {
      if (!has_drm_props) {
        /* lavapipe may intentionally omit the DRM identity extension:
         * it imports PRIME memory but never drives KMS itself. In the
         * unambiguous single-device CPU case, the compositor's real
         * import/readback probe is a stronger compatibility check. */
        found = num_phdevs == 1 &&
                phdev_props.deviceType == VK_PHYSICAL_DEVICE_TYPE_CPU;
        if (found) {
          wlr_log(WLR_INFO, "Using sole CPU Vulkan device without "
                            "VK_EXT_physical_device_drm");
        } else {
          wlr_log(WLR_DEBUG,
                  "  Ignoring physical device \"%s\": "
                  "VK_EXT_physical_device_drm not supported",
                  phdev_props.deviceName);
        }
      } else {
        dev_t primary_devid =
            makedev(drm_props.primaryMajor, drm_props.primaryMinor);
        dev_t render_devid =
            makedev(drm_props.renderMajor, drm_props.renderMinor);
        found = primary_devid == drm_stat.st_rdev ||
...
struct wlr_vk_device *vulkan_device_create(struct wlr_vk_instance *ini,
  ...
  bool has_external_semaphore_fd = check_extension(
      avail_ext_props, avail_extc, VK_KHR_EXTERNAL_SEMAPHORE_FD_EXTENSION_NAME);
  if (has_external_semaphore_fd) {
    ...
    vkGetPhysicalDeviceExternalSemaphoreProperties(phdev, &ext_semaphore_info,
                                                   &ext_semaphore_props);
  ...
  dev->implicit_sync_interop = exportable_semaphore && importable_semaphore &&
  ...
  dev->sampler_ycbcr_conversion =
      phdev_sampler_ycbcr_features.samplerYcbcrConversion;
  ...
  bool has_global_priority = check_extension(
      avail_ext_props, avail_extc, VK_EXT_GLOBAL_PRIORITY_EXTENSION_NAME);
  ...
  if (has_external_semaphore_fd) {
    ...
    load_device_proc(dev, "vkImportSemaphoreFdKHR",
                     &dev->api.vkImportSemaphoreFdKHR);
  }
