...
struct wlr_vk_texture *vulkan_get_texture(struct wlr_texture *wlr_texture) {
  ...
  struct wlr_vk_texture *texture =
      wl_container_of(wlr_texture, texture, wlr_texture);
  return texture;
}
...
static bool write_pixels(struct wlr_vk_texture *texture, uint32_t stride,
                         const pixman_region32_t *region, const void *vdata,
  ...
  struct wlr_vk_buffer_span span =
      vulkan_get_stage_span(renderer, bsize, format_info->bytes_per_block);
  ...
  for (int i = 0; i < rects_len; i++) {
    ...
    uint32_t packed_stride =
        (uint32_t)pixel_format_info_min_stride(format_info, width);
  ...
  vulkan_change_layout(cb, texture->image, old_layout, src_stage, src_access,
...
static bool vulkan_texture_update_from_buffer(struct wlr_texture *wlr_texture,
  ...
  ok = write_pixels(
      texture, stride, damage, data, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
...
static bool vulkan_texture_read_pixels(
  ...
  return vulkan_read_pixels(texture->renderer, texture->format->vk,
                            texture->image, options->format, options->stride,
                            src.width, src.height, src.x, src.y, 0, 0, p);
}

static uint32_t
vulkan_texture_preferred_read_format(struct wlr_texture *wlr_texture) {
  struct wlr_vk_texture *texture = vulkan_get_texture(wlr_texture);
...
static struct wlr_vk_texture *
vulkan_texture_create(struct wlr_vk_renderer *renderer, uint32_t width,
                      uint32_t height) {
...
struct wlr_vk_texture_view *vulkan_texture_get_or_create_view(
  ...
  VkImageViewCreateInfo view_info = {
      ...
      .components.a =
          texture->has_alpha || vulkan_format_is_ycbcr(texture->format)
  ...
  view->ds_pool = vulkan_alloc_texture_ds(texture->renderer,
                                          pipeline_layout->ds, &view->ds);
  if (!view->ds_pool) {
    free(view);
...
vulkan_texture_from_pixels(struct wlr_vk_renderer *renderer, uint32_t drm_fmt,
  ...
  if (width > fmt->shm.max_extent.width ||
      height > fmt->shm.max_extent.height) {
    wlr_log(WLR_ERROR,
            "Texture is too large to upload (%" PRIu32 "x%" PRIu32 " > %" PRIu32
            "x%" PRIu32 ")",
            width, height, fmt->shm.max_extent.width,
            fmt->shm.max_extent.height);
    return NULL;
  }

  struct wlr_vk_texture *texture =
      vulkan_texture_create(renderer, width, height);
  if (texture == NULL) {
    return NULL;
  }
...
VkImage vulkan_import_dmabuf(struct wlr_vk_renderer *renderer,
                             ...
                             VkDeviceMemory mems[static WLR_DMABUF_MAX_PLANES],
                             uint32_t *n_mems, bool for_render,
                             bool *using_mutable_srgb) {
  VkResult res;
  VkDevice dev = renderer->dev->dev;
  *n_mems = 0u;
  ...
  if (!mod) {
    ...
    wlr_log(WLR_ERROR,
            "Format %s (0x%08" PRIX32 ") can't be used with modifier "
            "%s (0x%016" PRIX64 ")",
            format_name, attribs->format, modifier_name, attribs->modifier);
    free(format_name);
    free(modifier_name);
    return VK_NULL_HANDLE;
  }
  ...
  if ((uint32_t)attribs->width > mod->max_extent.width ||
    ...
    wlr_log(WLR_ERROR,
            "DMA-BUF is too large to import (%" PRIi32 "x%" PRIi32 " > %" PRIu32
            "x%" PRIu32 ")",
  ...
  for (unsigned i = 0u; i < mem_count; ++i) {
    ...
    int mem = vulkan_find_mem_type(renderer->dev, 0,
                                   memr.memoryRequirements.memoryTypeBits &
                                       fdp.memoryTypeBits);
    if (mem < 0) {
...
static struct wlr_vk_texture *
vulkan_texture_from_dmabuf(struct wlr_vk_renderer *renderer,
  ...
  struct wlr_vk_texture *texture =
      vulkan_texture_create(renderer, attribs->width, attribs->height);
  if (texture == NULL) {
    return NULL;
  }
  ...
  texture->image =
      vulkan_import_dmabuf(renderer, attribs, texture->memories,
                           &texture->mem_count, false, &using_mutable_srgb);
  if (!texture->image) {
    goto error;
  }
...
static struct wlr_texture *
vulkan_texture_from_dmabuf_buffer(struct wlr_vk_renderer *renderer,
                                  struct wlr_buffer *buffer,
...
struct wlr_texture *
vulkan_texture_from_buffer(struct wlr_renderer *wlr_renderer,
                           struct wlr_buffer *buffer) {
  ...
  if (wlr_buffer_get_dmabuf(buffer, &dmabuf)) {
    return vulkan_texture_from_dmabuf_buffer(renderer, buffer, &dmabuf);
                                              ...
                                              &data, &format, &stride)) {
    struct wlr_texture *tex = vulkan_texture_from_pixels(
        renderer, format, stride, buffer->width, buffer->height, data);
    wlr_buffer_end_data_ptr_access(buffer);
    return tex;
  } else {
    return NULL;
