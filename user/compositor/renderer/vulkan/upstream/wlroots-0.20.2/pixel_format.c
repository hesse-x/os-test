...
static bool query_modifier_usage_support(
    struct wlr_vk_device *dev, VkFormat vk_format, VkFormat vk_format_variant,
...
static bool query_modifier_support(struct wlr_vk_device *dev,
                                   struct wlr_vk_format_props *props,
  ...
  for (uint32_t i = 0; i < modp.drmFormatModifierCount; ++i) {
    ...
    if ((m.drmFormatModifierTilingFeatures & render_features) ==
      ...
      if (query_modifier_usage_support(dev, props->format.vk,
                                       props->format.vk_srgb,
                                       vulkan_render_usage, &m, &p, &errmsg)) {
        supported = true;
        p.has_mutable_srgb = props->format.vk_srgb != 0;
      }
      if (!supported && props->format.vk_srgb) {
        supported = query_modifier_usage_support(
            dev, props->format.vk, 0, vulkan_render_usage, &m, &p, &errmsg);
      }

      if (supported) {
        props->dmabuf.render_mods[props->dmabuf.render_mod_count++] = p;
        wlr_drm_format_set_add(&dev->dmabuf_render_formats, props->format.drm,
                               m.drmFormatModifier);
        found = true;
      }
    } else {
    ...
    if ((m.drmFormatModifierTilingFeatures & features) == features) {
      ...
      if (query_modifier_usage_support(dev, props->format.vk,
                                       props->format.vk_srgb,
                                       vulkan_dma_tex_usage, &m, &p, &errmsg)) {
        supported = true;
        p.has_mutable_srgb = props->format.vk_srgb != 0;
      }
      if (!supported && props->format.vk_srgb) {
        supported = query_modifier_usage_support(
            dev, props->format.vk, 0, vulkan_dma_tex_usage, &m, &p, &errmsg);
      }

      if (supported) {
        props->dmabuf.texture_mods[props->dmabuf.texture_mod_count++] = p;
        wlr_drm_format_set_add(&dev->dmabuf_texture_formats, props->format.drm,
                               m.drmFormatModifier);
        found = true;
      }
    } else {
    ...
    if (errmsg != NULL) {
      snprintf(texture_status, sizeof(texture_status), "✗ texture (%s)",
               errmsg);
    } else {
...
void vulkan_format_props_query(struct wlr_vk_device *dev,
  ...
  const char *errmsg = "unknown error";
  if ((fmtp.formatProperties.optimalTilingFeatures & shm_tex_features) ==
          shm_tex_features &&
    ...
    if (supported) {
      ...
      wlr_drm_format_set_add(&dev->shm_texture_formats, format->drm,
  ...
  if (errmsg != NULL) {
    snprintf(shm_texture_status, sizeof(shm_texture_status), "✗ texture (%s)",
             errmsg);
  } else {
  ...
  if (modp.drmFormatModifierCount > 0) {
    add_fmt_props |=
        query_modifier_support(dev, &props, modp.drmFormatModifierCount);
  }
