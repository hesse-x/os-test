...
extern const VkImageUsageFlags vulkan_render_usage, vulkan_shm_tex_usage,
    vulkan_dma_tex_usage;
...
const struct wlr_vk_format_modifier_props *
vulkan_format_props_find_modifier(const struct wlr_vk_format_props *props,
                                  uint64_t mod, bool render);
...
struct wlr_vk_texture_view *
vulkan_texture_get_or_create_view(struct wlr_vk_texture *texture,
                                  const struct wlr_vk_pipeline_layout *layout,
                                  bool srgb);
...
vulkan_get_stage_span(struct wlr_vk_renderer *renderer, VkDeviceSize size,
...
struct wlr_vk_format_props *
vulkan_format_props_from_drm(struct wlr_vk_device *dev, uint32_t drm_format);
...
struct wlr_vk_texture {
  ...
  const struct wlr_vk_format *format;
  struct wlr_vk_command_buffer
      *last_used_cb; // to track when it can be destroyed
  bool dmabuf_imported;
  bool owned; // if dmabuf_imported: whether we have ownership of the image
  bool transitioned; // if dma_imported: whether we transitioned it away from
                     // preinit
...
struct wlr_vk_shared_buffer {
  struct wl_list link; // wlr_vk_renderer.stage.buffers or
                       // wlr_vk_command_buffer.stage_buffers
  VkBuffer buffer;
...
struct wlr_vk_color_transform {
  ...
  // if populated, carries the entire transform, other parameters are to be
  // ignored
  struct {
    size_t dim;
    VkImage image;
...
void vulkan_change_layout(VkCommandBuffer cb, VkImage img, VkImageLayout ol,
                          VkPipelineStageFlags srcs, VkAccessFlags srca,
                          VkImageLayout nl, VkPipelineStageFlags dsts,
                          VkAccessFlags dsta);

#if __STDC_VERSION__ >= 202311L

#define wlr_vk_error(fmt, res, ...)                                            \
  wlr_log(WLR_ERROR, fmt ": %s (%d)", vulkan_strerror(res),                    \
          res __VA_OPT__(, ) __VA_ARGS__)

#else

#define wlr_vk_error(fmt, res, ...)                                            \
  wlr_log(WLR_ERROR, fmt ": %s (%d)", vulkan_strerror(res), res, ##__VA_ARGS__)
