...
void matrix_invert(float out[static 9], float m[static 9]) {
  float a = m[0], b = m[1], c = m[2], d = m[3], e = m[4], f = m[5], g = m[6],
        h = m[7], i = m[8];
  ...
  float result[] = {
      inv_det * (e * i - f * h), inv_det * -(b * i - c * h),
      inv_det * (b * f - c * e), inv_det * -(d * i - f * g),
      inv_det * (a * i - c * g), inv_det * -(a * f - c * d),
      inv_det * (d * h - e * g), inv_det * -(a * h - b * g),
      inv_det * (a * e - b * d),
  };
