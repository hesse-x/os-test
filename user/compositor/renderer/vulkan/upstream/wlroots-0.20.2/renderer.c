...
static struct wlr_vk_render_format_setup *
find_or_create_render_setup(struct wlr_vk_renderer *renderer,
                            const struct wlr_vk_format *format,
                            bool has_blending_buffer, bool srgb);

static struct wlr_vk_descriptor_pool *
alloc_ds(struct wlr_vk_renderer *renderer, VkDescriptorSet *ds,
...
vulkan_alloc_texture_ds(struct wlr_vk_renderer *renderer,
                        VkDescriptorSetLayout ds_layout, VkDescriptorSet *ds) {
...
struct wlr_vk_descriptor_pool *
vulkan_alloc_blend_ds(struct wlr_vk_renderer *renderer, VkDescriptorSet *ds) {
  return alloc_ds(renderer, ds, VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT,
                  &renderer->output_ds_srgb_layout,
                  &renderer->output_descriptor_pools,
...
static void
destroy_render_format_setup(struct wlr_vk_renderer *renderer,
...
struct wlr_vk_buffer_span vulkan_get_stage_span(struct wlr_vk_renderer *r,
  ...
  if (!wl_list_empty(&r->stage.buffers)) {
    ...
    struct wlr_vk_shared_buffer *prev = wl_container_of(last_link, prev, link);
  ...
  res = vkMapMemory(r->dev->dev, buf->memory, 0, VK_WHOLE_SIZE, 0,
                    &buf->cpu_mapping);
  if (res != VK_SUCCESS) {
...
bool vulkan_submit_stage_wait(struct wlr_vk_renderer *renderer) {
  ...
  VkResult res =
      vkQueueSubmit(renderer->dev->queue, 1, &submit_info, VK_NULL_HANDLE);
  if (res != VK_SUCCESS) {
...
vulkan_format_props_from_drm(struct wlr_vk_device *dev, uint32_t drm_fmt) {
...
bool vulkan_wait_command_buffer(struct wlr_vk_command_buffer *cb,
  ...
  res = renderer->dev->api.vkWaitSemaphoresKHR(renderer->dev->dev, &wait_info,
                                               UINT64_MAX);
  if (res != VK_SUCCESS) {
...
static void release_command_buffer_resources(struct wlr_vk_command_buffer *cb,
  ...
  wl_list_for_each_safe(texture, texture_tmp, &cb->destroy_textures,
                        destroy_link) {
...
static struct wlr_vk_command_buffer *
get_command_buffer(struct wlr_vk_renderer *renderer) {
  VkResult res;

  uint64_t current_point;
  res = renderer->dev->api.vkGetSemaphoreCounterValueKHR(
...
vulkan_acquire_command_buffer(struct wlr_vk_renderer *renderer) {
...
bool vulkan_setup_two_pass_framebuffer(
  ...
  const struct wlr_vk_format_props *fmt =
      vulkan_format_props_from_drm(renderer->dev, dmabuf->format);
  assert(fmt);
  ...
  buffer->two_pass.render_setup =
      find_or_create_render_setup(renderer, &fmt->format, true, false);
  ...
  res = vkBindImageMemory(dev, buffer->two_pass.blend_image,
                          buffer->two_pass.blend_memory, 0);
  if (res != VK_SUCCESS) {
  ...
  res = vkCreateImageView(dev, &blend_view_info, NULL,
  ...
  buffer->two_pass.blend_attachment_pool =
      vulkan_alloc_blend_ds(renderer, &buffer->two_pass.blend_descriptor_set);
  if (!buffer->two_pass.blend_attachment_pool) {
...
bool vulkan_setup_one_pass_framebuffer(
  ...
  struct wlr_vk_render_buffer_out *out =
      srgb ? &buffer->srgb.out : &buffer->linear.out;
...
static struct wlr_vk_render_buffer *
create_render_buffer(struct wlr_vk_renderer *renderer,
  ...
  buffer->image =
      vulkan_import_dmabuf(renderer, &dmabuf, buffer->memories,
                           &buffer->mem_count, true, &using_mutable_srgb);
  if (!buffer->image) {
    goto error;
  }
...
static struct wlr_vk_render_buffer *
get_render_buffer(struct wlr_vk_renderer *renderer,
                  struct wlr_buffer *wlr_buffer) {
...
static bool
buffer_export_sync_file(struct wlr_vk_renderer *renderer,
                        struct wlr_buffer *buffer, uint32_t flags,
...
bool vulkan_sync_foreign_texture_acquire(
    struct wlr_vk_texture *texture,
    int sync_file_fds[static WLR_DMABUF_MAX_PLANES]) {
  return buffer_export_sync_file(texture->renderer, texture->buffer,
                                 DMA_BUF_SYNC_READ, sync_file_fds);
}

bool vulkan_sync_render_buffer_acquire(
    struct wlr_vk_render_buffer *render_buffer,
    int sync_file_fds[static WLR_DMABUF_MAX_PLANES]) {
  return buffer_export_sync_file(render_buffer->renderer,
                                 render_buffer->wlr_buffer, DMA_BUF_SYNC_WRITE,
                                 sync_file_fds);
}

static bool buffer_import_sync_file(struct wlr_buffer *buffer, uint32_t flags,
                                    int sync_file_fd) {
  struct wlr_dmabuf_attributes dmabuf = {0};
  if (!wlr_buffer_get_dmabuf(buffer, &dmabuf)) {
    wlr_log(WLR_ERROR, "wlr_buffer_get_dmabuf() failed");
  ...
  for (int i = 0; i < dmabuf.n_planes; i++) {
    if (!dmabuf_import_sync_file(dmabuf.fd[i], flags, sync_file_fd)) {
      return false;
    }
  }
...
bool vulkan_sync_render_pass_release(struct wlr_vk_renderer *renderer,
  ...
  int sync_file_fd = -1;
  res = renderer->dev->api.vkGetSemaphoreFdKHR(
      renderer->dev->dev, &get_fence_fd_info, &sync_file_fd);
  if (res != VK_SUCCESS) {
  ...
  if (pass->signal_timeline != NULL) {
    if (!wlr_drm_syncobj_timeline_import_sync_file(
            pass->signal_timeline, pass->signal_point, sync_file_fd)) {
      goto out;
    }
  } else {
    if (!buffer_import_sync_file(pass->render_buffer->wlr_buffer,
                                 DMA_BUF_SYNC_WRITE, sync_file_fd)) {
      goto out;
    }

    struct wlr_vk_render_pass_texture *pass_texture;
    wl_array_for_each(pass_texture, &pass->textures) {
      if (!buffer_import_sync_file(pass_texture->texture->buffer,
                                   DMA_BUF_SYNC_READ, sync_file_fd)) {
        goto out;
      }
    }
  }
...
vulkan_get_texture_formats(struct wlr_renderer *wlr_renderer,
...
static const struct wlr_drm_format_set *
vulkan_get_render_formats(struct wlr_renderer *wlr_renderer) {
  struct wlr_vk_renderer *renderer = vulkan_get_renderer(wlr_renderer);
...
static void vulkan_destroy(struct wlr_renderer *wlr_renderer) {
  ...
  wl_list_for_each_safe(setup, tmp_setup, &renderer->render_format_setups,
                        link) {
  ...
  wl_list_for_each_safe(pool, tmp_pool, &renderer->output_descriptor_pools,
                        link) {
    vkDestroyDescriptorPool(dev->dev, pool->pool, NULL);
    free(pool);
  }
  ...
  wl_list_for_each_safe(pipeline_layout, pipeline_layout_tmp,
    ...
    vkDestroySamplerYcbcrConversion(dev->dev, pipeline_layout->ycbcr.conversion,
                                    NULL);
  ...
  vkDestroyDescriptorSetLayout(dev->dev, renderer->output_ds_lut3d_layout,
                               NULL);
...
bool vulkan_read_pixels(struct wlr_vk_renderer *vk_renderer,
  ...
  if (!pixel_format_info) {
    wlr_log(WLR_ERROR,
            "vulkan_read_pixels: could not find pixel format info "
            "for DRM format 0x%08x",
            drm_format);
    return false;
  ...
  const struct wlr_vk_format *wlr_vk_format =
      vulkan_get_format_from_drm(drm_format);
  if (!wlr_vk_format) {
    wlr_log(WLR_ERROR,
            "vulkan_read_pixels: no vulkan format "
            "matching drm format 0x%08x available",
            drm_format);
    return false;
  }
  VkFormat dst_format = wlr_vk_format->vk;
  VkFormatProperties dst_format_props = {0}, src_format_props = {0};
  vkGetPhysicalDeviceFormatProperties(vk_renderer->dev->phdev, dst_format,
                                      &dst_format_props);
  vkGetPhysicalDeviceFormatProperties(vk_renderer->dev->phdev, src_format,
                                      &src_format_props);
  ...
  if (use_cached) {
    ...
    if (mem_type < 0) {
      wlr_log(WLR_ERROR,
              "vulkan_read_pixels: could not find adequate memory type");
      goto destroy_image;
    }
  ...
  vulkan_change_layout(
      cb, dst_image, VK_IMAGE_LAYOUT_UNDEFINED, VK_PIPELINE_STAGE_TRANSFER_BIT,
      0, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
      VK_ACCESS_TRANSFER_WRITE_BIT);
  vulkan_change_layout(
      cb, src_image, VK_IMAGE_LAYOUT_GENERAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
  ...
  if (blit_supported) {
    ...
    vkCmdBlitImage(cb, src_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                   dst_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
                   &image_blit_region, VK_FILTER_NEAREST);
  } else {
    wlr_log(WLR_DEBUG, "vulkan_read_pixels: blit unsupported, falling back to "
                       "vkCmdCopyImage.");
  ...
  vulkan_change_layout(cb, dst_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                       VK_PIPELINE_STAGE_TRANSFER_BIT,
                       VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_GENERAL,
                       VK_PIPELINE_STAGE_TRANSFER_BIT, 0);
  vulkan_change_layout(cb, src_image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                       VK_PIPELINE_STAGE_TRANSFER_BIT,
  ...
  VkImageSubresource img_sub_res = {
      .aspectMask = VK_IMAGE_ASPECT_COLOR_BIT, .arrayLayer = 0, .mipLevel = 0};
  ...
  if (pack_stride == stride && dst_x == 0) {
    ...
    for (size_t i = 0; i < height; ++i) {
      memcpy(p + i * stride + dst_x * bytes_per_pixel, d + i * pack_stride,
...
static struct wlr_render_pass *
vulkan_begin_buffer_pass(struct wlr_renderer *wlr_renderer,
                         struct wlr_buffer *buffer,
                         const struct wlr_buffer_pass_options *options) {
  struct wlr_vk_renderer *renderer = vulkan_get_renderer(wlr_renderer);

  struct wlr_vk_render_buffer *render_buffer =
      get_render_buffer(renderer, buffer);
  if (!render_buffer) {
  ...
  struct wlr_vk_render_pass *render_pass =
      vulkan_begin_render_pass(renderer, render_buffer, options);
  if (render_pass == NULL) {
    return NULL;
  }
...
static bool
pipeline_layout_key_equals(const struct wlr_vk_pipeline_layout_key *a,
...
setup_get_or_create_pipeline(struct wlr_vk_render_format_setup *setup,
  ...
  struct wlr_vk_pipeline_layout *pipeline_layout =
      get_or_create_pipeline_layout(renderer, &key->layout);
  if (!pipeline_layout) {
    return NULL;
  }
...
static bool
init_blend_to_output_pipeline(struct wlr_vk_renderer *renderer, VkRenderPass rp,
...
ycbcr_model_from_wlr(enum wlr_color_encoding encoding) {
  switch (encoding) {
...
get_or_create_pipeline_layout(struct wlr_vk_renderer *renderer,
  ...
  if (key->ycbcr.format) {
    ...
    res = vkCreateSamplerYcbcrConversion(renderer->dev->dev,
                                         &conversion_create_info, NULL,
                                         &pipeline_layout->ycbcr.conversion);
    if (res != VK_SUCCESS) {
    ...
    assert(key->ycbcr.encoding == WLR_COLOR_ENCODING_NONE ||
           key->ycbcr.encoding == WLR_COLOR_ENCODING_IDENTITY);
    assert(key->ycbcr.range == WLR_COLOR_RANGE_NONE ||
           key->ycbcr.range == WLR_COLOR_RANGE_FULL);
  }

  res = vkCreateSampler(renderer->dev->dev, &sampler_create_info, NULL,
                        &pipeline_layout->sampler);
  if (res != VK_SUCCESS) {
    wlr_vk_error("vkCreateSampler", res);
    free(pipeline_layout);
    return NULL;
  ...
  if (!init_tex_layouts(renderer, pipeline_layout->sampler,
                        &pipeline_layout->ds, &pipeline_layout->vk)) {
    free(pipeline_layout);
    return NULL;
  }
 ...
 * the sampler, a valid descriptor set should be bound. Create that here, linked
 * to a 1x1x1 image.
 */
static bool init_dummy_images(struct wlr_vk_renderer *renderer) {
  ...
  res =
      vkBindImageMemory(dev, renderer->dummy3d_image, renderer->dummy3d_mem, 0);
  if (res != VK_SUCCESS) {
    wlr_vk_error("vkBindMemory failed", res);
    return false;
  }
  ...
  renderer->output_ds_lut3d_dummy_pool =
      vulkan_alloc_texture_ds(renderer, renderer->output_ds_lut3d_layout,
...
find_or_create_render_setup(struct wlr_vk_renderer *renderer,
  ...
  if (use_blending_buffer) {
    ...
    if (!init_blend_to_output_pipeline(
            renderer, setup->render_pass, renderer->output_pipe_layout,
            &setup->output_pipe_pq,
            WLR_VK_OUTPUT_TRANSFORM_INVERSE_ST2084_PQ)) {
      goto error;
    }
    if (!init_blend_to_output_pipeline(
            renderer, setup->render_pass, renderer->output_pipe_layout,
            &setup->output_pipe_gamma22,
            WLR_VK_OUTPUT_TRANSFORM_INVERSE_GAMMA22)) {
      goto error;
    }
    if (!init_blend_to_output_pipeline(
            renderer, setup->render_pass, renderer->output_pipe_layout,
            &setup->output_pipe_bt1886,
            WLR_VK_OUTPUT_TRANSFORM_INVERSE_BT1886)) {
      goto error;
    }
  } else {
  ...
  if (!setup_get_or_create_pipeline(
          setup, &(struct wlr_vk_pipeline_key){
...
struct wlr_renderer *
vulkan_renderer_create_for_device(struct wlr_vk_device *dev) {
  struct wlr_vk_renderer *renderer;
  VkResult res;
  ...
  renderer->wlr_renderer.color_encodings = WLR_COLOR_ENCODING_BT601 |
                                           WLR_COLOR_ENCODING_BT709 |
                                           WLR_COLOR_ENCODING_BT2020;

  uint64_t cap_syncobj_timeline;
  if (dev->drm_fd >= 0 && drmGetCap(dev->drm_fd, DRM_CAP_SYNCOBJ_TIMELINE,
                                    &cap_syncobj_timeline) == 0) {
    renderer->wlr_renderer.features.timeline =
        dev->sync_file_import_export && cap_syncobj_timeline != 0;
  }
  ...
  res =
      vkCreateCommandPool(dev->dev, &cpool_info, NULL, &renderer->command_pool);
  if (res != VK_SUCCESS) {
...
struct wlr_renderer *wlr_vk_renderer_create_with_drm_fd(int drm_fd) {
  ...
  if (dev->drm_fd < 0) {
    /* A sole CPU Vulkan device without VK_EXT_physical_device_drm has no
     * discoverable node. Keep an independent ref to the selected backend
     * node for format/sync queries; Vulkan still only imports PRIME fds. */
    dev->drm_fd = fcntl(drm_fd, F_DUPFD_CLOEXEC, 0);
    if (dev->drm_fd < 0) {
      wlr_log_errno(WLR_ERROR,
                    "Failed to duplicate DRM fd for Vulkan renderer");
      vulkan_device_destroy(dev);
      return NULL;
    }
  }
...
VkPhysicalDevice
wlr_vk_renderer_get_physical_device(struct wlr_renderer *renderer) {
  struct wlr_vk_renderer *vk_renderer = vulkan_get_renderer(renderer);
