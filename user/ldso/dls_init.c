...
static struct link_map *find_loaded(const char *soname) {
    for (int i = 0; i < g_loaded_cnt; i++) {
        if (strcmp(g_loaded[i].soname, soname) == 0) return g_loaded[i].map;
    }
    return NULL;
}
...
static void register_loaded(const char *soname, struct link_map *m) {
    ...
    // 拷贝 soname（截断到 63 字节）
    int i = 0;
    while (soname[i] && i < 63) { g_loaded[g_loaded_cnt].soname[i] = soname[i]; i++; }
...
static struct link_map *load_one(const char *soname) {
    ...
    // （ld.so 测试 stub）。对齐 Linux DT_RPATH 思路；当前不解析 DT_RPATH/DT_RUNPATH
    // （ld.md §8.4 已知边界），硬编码前缀已满足所有库放 /lib/ 或 /test/lib/。
    ...
    *dp = '\0';
    ...
    long fd;
    __asm__ volatile("syscall" : "=a"(fd) : "a"(SYS_OPEN), "D"((int64_t)(uintptr_t)lib_path), "S"((int64_t)O_RDONLY) : "rcx", "r11");
    if (fd < 0) {
        // 回退 /test/lib/<soname>
        ...
        *dp = '\0';
        ...
        long rc;
        __asm__ volatile("syscall" : "=a"(rc) : "a"(SYS_CLOSE), "D"((int64_t)fd) : "rcx", "r11");
    }
    ...
    // 填 soname
    int i = 0;
    while (soname[i] && i < 63) { m->soname[i] = soname[i]; i++; }
    m->soname[i] = '\0';
    m->l_next = NULL;
    m->l_prev = NULL;
    return m;
...
static struct link_map *load_recursive(const char *soname, struct link_map *tail) {
    if (find_loaded(soname)) return tail;          // 去重（兼闭环收敛）
