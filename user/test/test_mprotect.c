...
// Covers plan1.md §1.5 acceptance (mprotect = gap1 hard acceptance):
...
//     MAPERR — §1.3.3 PF fix: PROTNONE leaf is a valid mapping with no
//     access), then back to RW -> write/read OK
//  4. PROT_NONE then munmap -> re-alloc no leak (physical reclaim via
//     §1.3.3 unmap_user_pages/proc.c leaf reclaim using pte_present())
//
...
// NOT via a shared global — after fork the child's BSS is a COW-private copy,
// so a write in the child is invisible to the parent. The parent reaps it
// with waitpid and reads WEXITSTATUS; if the child was killed by SIGSEGV
// (handler never installed / second fault) WIFSIGNALED is true and the parent
// treats the code as 0 (no capture).
...
// the parent can read it via WEXITSTATUS. The signal has already been
// observed, so _exit immediately (no need to restore SIG_DFL).
...
void test_mprotect_rw_to_rx_exec(void) {
  ...
  // Flip to R|X. The RX transition itself must take effect; on CPUs without
  // PCID or with stale kernel TLB entries, a missing invlpg would let the
  // old RW mapping linger.
...
void test_sysconf_pagesize(void) {
  // closures.c uses sysconf(_SC_PAGESIZE) to size the executable mmap region
  // for ffi_closure trampolines; a wrong value breaks the whole closure path.
  // Must equal getpagesize()/PAGE_SIZE.
