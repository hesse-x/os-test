...
/* 1. 只读 fs (sysfs) 上 O_CREAT 应失败(EACCES:parent->i_op->create 为 NULL,对齐 Linux vfs_create)。 */
...
/* 2. 只读 fs 无 setattr:truncate 应失败(EISDIR:/sys 是目录,对齐 Linux truncate 优先判 INODE_REGULAR)。 */
void test_vfs_dispatch_sysfs_truncate_denied(void) {
  errno = 0;
  int rc = truncate("/sys", 0);
  TEST_ASSERT_EQUAL_INT(-1, rc);
  TEST_ASSERT_EQUAL_INT(EISDIR, errno); /* 目录 truncate → EISDIR(非旧的 EINVAL) */
}

/* 3. fat32 上 O_CREAT|O_TRUNC 建文件并截 0(经 fat32_dir_create + fat32_setattr)。 */
