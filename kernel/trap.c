...
int64_t sys_pipe(int64_t arg1, int64_t _u1, int64_t _u2, int64_t _u3, int64_t _u4, int64_t _u5) {
    ...
    spinlock_t *fdlk = &proc->mm->files->fd_lock;
    spin_lock(fdlk);
    int read_fd = alloc_fd(proc->mm->files, 3);
    int write_fd = (read_fd >= 0) ? alloc_fd(proc->mm->files, read_fd + 1) : -EMFILE;
    ...
    fr->flags = O_RDONLY;
    fr->pipe = p;
    fd_install(proc->mm->files, read_fd, fr);
...
int64_t sys_dup2(int64_t arg1, int64_t arg2, int64_t _u1, int64_t _u2, int64_t _u3, int64_t _u4) {
    ...
    // Check old_fd valid
    struct file *old_f = fd_lookup(proc->mm->files, old_fd);
    if (!old_f) {
        spin_unlock(fdlk);
        return (int64_t)-EBADF;
    }
