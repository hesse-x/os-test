...
struct acpi_madt_result g_madt = {};
struct acpi_mcfg_result g_mcfg = {};
...
void *acpi_find_table(const char signature[4]) {
  if (xsdt_virt == 0) return nullptr;

  for (uint32_t i = 0; i < xsdt_entry_count; i++) {
    uint64_t addr;
    if (xsdt_entry_size == 8) {
      addr = *(const uint64_t *)(xsdt_virt + sizeof(struct acpi_sdt_header) + i * 8);
    } else {
      addr = *(const uint32_t *)(xsdt_virt + sizeof(struct acpi_sdt_header) + i * 4);
    }
    uint64_t table_virt = addr + VMA_BASE;
    const struct acpi_sdt_header *t = (const struct acpi_sdt_header *)table_virt;
...
static void parse_madt(const struct acpi_madt *madt) {
  ...
  while (offset < madt->header.length) {
    const struct acpi_madt_entry *e =
        (const struct acpi_madt_entry *)((const uint8_t *)madt + offset);
    if (e->length == 0) break;

    if (e->type == 0 && g_madt.ncpus < 4) {
      const struct acpi_madt_lapic_entry *lapic =
      ...
      const struct acpi_madt_ioapic_entry *ioapic =
...
static void parse_mcfg(const struct acpi_mcfg *mcfg) {
  size_t offset = sizeof(struct acpi_mcfg);
  if (offset + sizeof(struct acpi_mcfg_entry) > mcfg->header.length) {
    serial_puts("acpi: MCFG has no entries\n");
    return;
  }
  const struct acpi_mcfg_entry *entry =
      (const struct acpi_mcfg_entry *)((const uint8_t *)mcfg + offset);
...
void acpi_init(uint64_t rsdp_phys) {
  ...
  const struct acpi_sdt_header *sdt_hdr = (const struct acpi_sdt_header *)xsdt_virt;
  xsdt_entry_count = (sdt_hdr->length - sizeof(struct acpi_sdt_header)) / xsdt_entry_size;
