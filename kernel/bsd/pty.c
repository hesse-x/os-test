...
// ===================== pty_master_read =====================
int64_t pty_master_read(struct pty *pty, xtask_t *proc,
                        void *buf, size_t len) {
...
// ===================== pty_master_write =====================
int64_t pty_master_write(struct pty *pty, xtask_t *proc,
...
// ===================== pty_slave_read =====================
int64_t pty_slave_read(struct pty *pty, xtask_t *proc,
                        void *buf, size_t len) {
...
int64_t pty_slave_write(struct pty *pty, xtask_t *proc,
                         const void *buf, size_t len) {
    if (pty->master_refs == 0) return -EPIPE;
...
long pty_ioctl(struct pty *pty, uint32_t cmd, void *arg) {
    ...
    case TIOCSWINSZ: // 0x5414
        {
            ...
            if ((old_ws.ws_row != pty->t_winsize.ws_row || old_ws.ws_col != pty->t_winsize.ws_col)
                ...
                for (int p = 0; p < MAX_PROC; p++) {
                    if (tasks[p] && tasks[p]->pid == p && tasks[p]->proc->pgid == pty->t_pgid && tasks[p]->proc->sid == pty->t_sid) {
    ...
    case TIOCSCTTY: // 0x540E — set controlling terminal
        {
            if (current_proc->sid != current_task->pid) return -EPERM;
            int force = (int)(uintptr_t)arg;
            if (current_proc->ctty && !force) return -EINVAL;
