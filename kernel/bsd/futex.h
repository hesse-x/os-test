...
#define FUTEX_WAIT 0
#define FUTEX_WAKE 1
// musl pthread condvar hand-off (cond_timedwait.c unlock_requeue): moves the
// next waiter blocked on the barrier from the cv to the mutex, saving a
// user-space round-trip and avoiding a thundering herd. Without it, musl's
// fallback retries REQUEUE|PRIVATE → REQUEUE (not downgrading to WAKE); if
// both return ENOSYS it silently drops wakeups → with multiple waiters
// signalled one by one, the hand-off break point blocks a waiter forever until
// a broadcast unblocks it. So this is a correctness feature, not perf.
#define FUTEX_REQUEUE 3
#define FUTEX_CMP_REQUEUE                                                      \
  4 // like REQUEUE, but checks *uaddr1==cmpval before delivery; mismatch →
    // -EAGAIN
