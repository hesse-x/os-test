...
#define STACK_KV(sp_user)                                                      \
  ...
  // 4. Align the final entry stack, including argc/argv/envp/auxv.  The
  // pointer-table size changes by 8 bytes when argc + envc changes parity.
  while ((sp_user % 16) != 0)
    sp_user--;

  // 5. auxv (12 pairs + AT_NULL)
  // Mirrors the execve auxv (proc.c): 6 base pairs
  // (PHDR/PHENT/PHNUM/ENTRY/PAGESZ/ RANDOM/EXECFN) + 6 musl-required pairs
  // (UID/EUID/GID/EGID/SECURE/HWCAP). The kernel-init process is single-user
  // root, so all ids are 0 and AT_SECURE=0. AT_BASE is omitted (init is static,
  // no PT_INTERP). musl's static-PIE/__libc_start_main path reads these
  // (src/env/__libc_start_main.c:41-42); keep parity with execve.
  int auxc = 12;
