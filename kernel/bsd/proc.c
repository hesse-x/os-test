...
#define STACK_KV(sp_user)                                                      \
  ...
  // 5. Align the final entry stack, including argc/argv/envp/auxv.  The
  // pointer-table size changes by 8 bytes when argc + envc changes parity.
  while ((sp_user % 16) != 0)
    sp_user--;

  // 6. Write auxv (15 pairs + AT_NULL)
  // 8 base pairs (PHDR/PHENT/PHNUM/ENTRY/BASE/PAGESZ/RANDOM/EXECFN) + 7
  // musl-required pairs (UID/EUID/GID/EGID/PLATFORM/SECURE/HWCAP). musl's fused
  // loader reads these at
  // __dls3 (dynlink.c:1468-1471): HWCAP → __hwcap, UID/EUID/GID/EGID/SECURE →
  // libc.secure (non-zero ⇒ secure-execution mode, dropping LD_*). Absent pairs
  // make musl treat every exec as secure. Single-user system: all ids default
  // 0, AT_SECURE = (euid!=uid || egid!=gid) = 0, AT_HWCAP = 0 (no CPUID infra
  // yet). AT_PLATFORM is not read by the loader, but musl's secure-mode
  // heuristic (dynlink.c:1470) gates libc.secure on (aux[0]&0x7800)==0x7800 —
  // the bitmask of set AT_* entries must cover bits 11..15
  // (UID/EUID/GID/EGID/PLATFORM). Omitting AT_PLATFORM clears bit 15 →
  // libc.secure forced to 1 → LD_LIBRARY_PATH (and LD_PRELOAD) silently
  // dropped, so the ld_* chain tests can't find liba/libb. Value 0 is safe:
  // musl never dereferences aux[AT_PLATFORM] in the loader path.
  int auxc = 15;
  uint64_t table_size = (uint64_t)(auxc + 1) * 16 + (uint64_t)(envc + 1) * 8 +
                        (uint64_t)(argc + 1) * 8 + 8;
  if (table_size % 16 != 0)
    sp_user -= 8;
  sp_user -= (auxc + 1) * 16;
