...
void proc_free(proc_t *bp) {
    if (!bp) return;
    if (bp->files) files_put(bp->files);
...
int64_t sys_fork(int64_t a1, int64_t a2, int64_t a3,
    ...
    if (!child_bp) { mm_put(child_mm); spin_unlock(&tasks_lock); return (int64_t)-ENOMEM; }
    child->proc = child_bp;
...
int64_t sys_execve(int64_t a1, int64_t a2, int64_t a3,
    ...
    if (!fd_lookup(proc->proc->files, fd) || fd_lookup(proc->proc->files, fd)->type != FD_REGULAR) {
