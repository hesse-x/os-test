...
int irq_has_handler(int irq) {
    if (irq < 0 || irq >= MAX_IRQ_HANDLERS) return 0;
...
static void resolve_cow_fault(xtask_t *task, uint64_t *pte, uint64_t fault_addr) {
    ...
    if (!new_page) {
        if (force_sig_hook) force_sig_hook(task, SIGSEGV, SEGV_MAPERR, (void *)fault_addr);
        return;
    }
...
void trap_dispatch(trapframe_t *tf) {
  ...
  if (tf->cs == 0x2B) {
    ...
    case 13:  // #GP general protection
        ...
        break;
    case 14:  // #PF page fault — COW already resolved above; only genuine faults reach here
        {
            uint64_t fault_addr = read_cr2();
    ...
    if (force_sig_hook) force_sig_hook(current_task, sig, si_code, si_addr);
...
void xcall_dispatch(trapframe_t *tf) {
    ...
    if (nr >= 0 && nr < NR_XCORE_SYSCALL && xcore_syscall_table[nr]) {
        tf->rax = xcore_syscall_table[nr](
            tf->rdi, tf->rsi, tf->rdx, tf->r10, tf->r8, tf->r9);
    } else if (nr >= 0) {
        // All other syscalls → BSD layer
        tf->rax = syscall_dispatch_hook ? syscall_dispatch_hook(tf) : (int64_t)-ENOSYS;
    } else {
