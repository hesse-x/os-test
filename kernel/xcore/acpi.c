...
acpi_madt_result g_madt = {0, 0, 0, 0, {0}, 0, {{0}}};
acpi_mcfg_result g_mcfg = {0, 0, 0, 0};
...
acpi_find_table(const char signature[4]) {
  ...
  for (uint32_t i = 0; i < xsdt_entry_count; i++) {
    ...
    if (xsdt_entry_size == 8) {
      addr = *(const uint64_t *)(xsdt_virt + sizeof(acpi_sdt_header) + i * 8);
    } else {
      addr = *(const uint32_t *)(xsdt_virt + sizeof(acpi_sdt_header) + i * 4);
    }
    uint64_t table_virt = addr + VMA_BASE;
    const acpi_sdt_header *t = (const acpi_sdt_header *)table_virt;
...
static void parse_madt(const acpi_madt *madt) {
  g_madt.lapic_base = madt->local_apic_address;
  g_madt.ncpus = 0;

  size_t offset = sizeof(acpi_madt);
  while (offset < madt->header.length) {
    const acpi_madt_entry *e =
        (const acpi_madt_entry *)((const uint8_t *)madt + offset);
    if (e->length == 0)
      break;

    if (e->type == 0 && g_madt.ncpus < 4) {
      const acpi_madt_lapic_entry *lapic = (const acpi_madt_lapic_entry *)e;
      if (lapic->flags & 1) {
      ...
      const acpi_madt_iso_entry *iso = (const acpi_madt_iso_entry *)e;
      if (iso->bus == 0) { // only ISA overrides
        acpi_iso_override *o = &g_madt.iso[g_madt.num_iso];
        o->irq = iso->irq;
        o->gsi = iso->gsi;
...
static void parse_mcfg(const acpi_mcfg *mcfg) {
  size_t offset = sizeof(acpi_mcfg);
  if (offset + sizeof(acpi_mcfg_entry) > mcfg->header.length) {
    printk(LOG_WARN, "acpi: MCFG has no entries\n");
    return;
  }
  const acpi_mcfg_entry *entry =
      (const acpi_mcfg_entry *)((const uint8_t *)mcfg + offset);
...
acpi_init(uint64_t rsdp_phys) {
  ...
  const acpi_sdt_header *sdt_hdr = (const acpi_sdt_header *)xsdt_virt;
  xsdt_entry_count =
      (sdt_hdr->length - sizeof(acpi_sdt_header)) / xsdt_entry_size;
