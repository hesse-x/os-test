...
static inline void spin_unlock(spinlock *lk) __releases(lk) {
...
static inline void spin_lock_irqsave(spinlock *lk, uint64_t *flags)
...
static inline void spin_unlock_irqrestore(spinlock *lk, uint64_t flags)
    __releases(lk) {
  spin_unlock(lk);
  __asm__ volatile("pushq %0; popfq" : : "r"(flags));
}

static inline int spin_trylock(spinlock *lk) __acquires(lk) {
  return __atomic_exchange_n(&lk->locked, 1, __ATOMIC_ACQUIRE) == 0;
}

// trylock failure must popfq to restore original IF state, avoiding CLI state
// leak
