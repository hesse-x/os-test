...
void synchronize_rcu(void) {
  ...
  // in a quiescent state. Published at new_gen (not new_gen - 1) so a concurrent
  // writer on another CPU, which waits for cpu_gen >= its own new_gen, sees this
  // CPU as having passed this grace period. Without this, a busy-waiting
  // synchronize_rcu would never see its own cpu_gen advance (it never passes
  // through rcu_read_unlock while spinning).
  ...
  // CS ends. Therefore requiring cpu_gen[i] >= new_gen forces the writer to wait
  // until each CPU has either:
