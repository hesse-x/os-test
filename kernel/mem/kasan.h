...
#define KASAN_MEM_TO_SHADOW(addr)  ((uint8_t *)(((uint64_t)(addr) >> KASAN_SHADOW_SCALE) + KASAN_SHADOW_OFFSET))
#define KASAN_SHADOW_TO_MEM(shadow) ((void *)(((uint64_t)(shadow) - KASAN_SHADOW_OFFSET) << KASAN_SHADOW_SCALE))
...
#define KASAN_SHADOW_SIZE   ((uint64_t)KASAN_SHADOW_END - (uint64_t)KASAN_SHADOW_START)
...
static inline void kasan_unpoison_shadow(const void *a, size_t s) { (void)a; (void)s; }
static inline void kasan_poison_shadow(const void *a, size_t s, uint8_t v) { (void)a; (void)s; (void)v; }
...
static inline void kasan_bfc_free(const void *a, size_t s) { (void)a; (void)s; }
