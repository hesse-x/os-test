...
5. `kernel_main`：`init_mem` → `isr_init`（内含第二次 `gdt_init`，BSP 的 `smp_init_cpu` + `smp_apply_cpu`，`apic_init`）→ `kernel_init_finish`（禁用 bump）→ `proc_init` + `init_idle_proc` → `smp_boot_aps`（启动所有 AP）→ 从磁盘加载 shell.elf → `process_create_elf` → `schedule` → idle 循环
...
- **Per-CPU 数据**：`cpu_local_t`（`arch/x64/smp.h`）通过 SWAPGS/GS_BASE MSR 机制访问，包含 cpu_id、apic_id、`_cur_proc`、lapic_base、kernel_stack、tss_rsp0、run_count。`current_proc` 宏展开为 `get_cpu_local()->_cur_proc`
- **Per-CPU GDT/TSS**：每 CPU 独立的 7 项 GDT + TSS（`per_cpu_gdt[MAX_CPUS][7]`、`per_cpu_tss[MAX_CPUS]`），在 `smp_init_cpu()` 中初始化，`smp_apply_cpu()` 中通过 `lgdt` + `swapgs` + `ltr` 加载
