...
- **最小内核**：内核仅包含调度、内存管理、中断分发、系统调用等不可替代的机制。驱动（kbd/fb/ata）当前在内核空间，但应逐步移至用户态。
- **用户态服务**：shell 等应用作为独立用户进程从磁盘加载，拥有独立地址空间（每进程 PML4），通过 syscall 与内核通信。
- **系统调用即 IPC**：当前 4 个 syscall（putc/getpid/yield/getc）是用户态与内核的唯一通信接口。新增功能应优先考虑扩展 syscall 接口而非在内核内实现策略。
- **进程隔离**：每个用户进程拥有独立地址空间（代码 0x400000 起，栈 0x00007FFFFFFFD000），PML4[511] 共享内核映射，其余独立。
...
5. `kernel_main`：`init_mem` → `isr_init`（内含第二次 `gdt_init`，BSP 的 `smp_init_cpu` + `smp_apply_cpu`，`enable_nx`，`idt_install` + `setup_syscall`，`apic_init`）→ `kernel_init_finish`（禁用 bump）→ `proc_init` + `init_idle_proc` → `smp_boot_aps`（启动所有 AP）→ 从磁盘加载 shell.elf → `process_create_elf` → `schedule` → idle 循环
...
- **Per-CPU 数据**：`cpu_local_t`（`arch/x64/smp.h`）通过 SWAPGS/GS_BASE MSR 机制访问，包含 cpu_id、apic_id、`_cur_proc`、lapic_base、kernel_stack、tss_rsp0、run_count。`current_proc` 宏展开为 `get_cpu_local()->_cur_proc`。SYSCALL 入口通过 `%gs:32` 读 `tss_rsp0` 切换内核栈（`kernel_stack` 不随 schedule 更新）
- **Per-CPU GDT/TSS**：每 CPU 独立的 7 项 GDT + TSS（`per_cpu_gdt[MAX_CPUS][7]`、`per_cpu_tss[MAX_CPUS]`、`per_cpu_ist_stack[MAX_CPUS][3]`），在 `smp_init_cpu()` 中初始化，`smp_apply_cpu()` 中通过 `lgdt` + `swapgs` + `ltr` 加载
...
- **CPU 挑选**：`pick_cpu()`（`kernel/proc.cc`）基于 `run_count` 做简单负载均衡
- **当前限制**：AP 进入 idle 循环（hlt），不参与用户进程调度；无 spinlock/原子操作/内存屏障，共享数据（`procs[]`、`tick`、`kbd_buffer`）在多核下无锁保护；无运行时 IPI（reschedule/TLB shootdown）
...
- **创建方式**：`process_create(entry)` 使用硬编码 `init_code[]`（使用 `syscall` 指令，RAX=syscall#，RDI=arg1）；`process_create_elf(elf_data, size)` 从 ELF64 加载
- **地址空间**：代码从 0x400000 起，用户栈页面映射在 0x00007FFFFFFFD000，RSP 初始值 0x00007FFFFFFFE000（页面顶端），每进程独立 PML4（共享 PML4[511] 内核映射）
- **系统调用**：4个（putc/getpid/yield/getc），通过 `syscall` 指令触发（EFER.SCE），rax=syscall#，参数通过 rdi/rsi/rdx/r10/r8 传递
...
- **Shell 构建**：`g++ -m64 -ffreestanding -nostdlib -fno-builtin -fno-pie -fno-stack-protector -I. -c user/shell.cc` + `ld -m elf_x86_64 -Ttext 0x400000`。注意 `-fno-pie`（固定地址链接）和 `-I.`（允许 include `arch/x64/utils.h` 中的 syscall 封装，现在使用 `syscall` 指令而非 `int $0x80`）
