...
/* ringbuf_header — SHM ring buffer header (design 3.5).
 * The producer (user-space evdev) writes slot[head] and advances head
 * unconditionally (overwrite-oldest via ringbuf_push); consumers read via
 * the kernel ring_t path with per-fd f->offset cursors. No tail field —
 * each consumer's read position lives in its own file object. */
