...
constexpr size_t header_length = sizeof(multiboot_header) + sizeof(multiboot_header_tag_framebuffer) + sizeof(multiboot_header_tag);
...
extern "C" {
static const uint8_t stack_bottom[8192] __attribute__((aligned(16))) = {0};

void _start() __attribute__((externally_visible, noreturn, naked));
void _start() {
    uintptr_t stack_top = (uintptr_t)(stack_bottom + sizeof(stack_bottom));
    __asm__ volatile (
